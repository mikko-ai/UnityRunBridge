...
namespace Mk.UnityAgentBridge.Editor.Hierarchy
{
    ...
    /// （基础类型给值，引用类型只给对象名/null，不递归）+ INodeEnricher 检查增强。
    /// Phase 3：Button onClickListeners 等 UGUI 特化由 Adapter Enricher 追加。
    /// </summary>
    internal static class NodeInspector
    {
        public static JsonValue BuildInspect(Transform transform, IBridgeServiceResolver services = null)
        {
            services = BridgeServices.Current(services);
            GameObject go = transform.gameObject;
            JsonValue node = NodeSerializer.BuildSummary(transform, services);
            ...
            NodeSerializer.ApplyEnrichersInspection(go, node, services);
            return node;
        }
