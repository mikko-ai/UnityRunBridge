...
namespace Mk.UnityAgentBridge.Editor.Recording
{
    ...
    /// </summary>
    // 不再用 [InitializeOnLoad] 自启，改由 RecordingBridgeModule 的生命周期
    ...
    internal static class RecordingController
    {
        ...
        /// <summary>由 RecordingBridgeModule 生命周期在 Host Start 时调用，触发静态构造函数
        /// 注册 TempObjectRegistry 清理钩子（等价于原先的 [InitializeOnLoad]）。</summary>
        internal static void EnsureInitialized()
        {
        }

        internal static object Start(BridgeRequestContext ctx, IBridgeServiceResolver services = null)
        {
            ...
            // 用显式 resolver 判断，避免 HasInputBackend 读到过期的全局 Active。
            if (!RecordingListener.TrySelectPointerBackend(services, out _))
            {
            ...
            RecordingListener.StartListening(action => AppendAction(actionsPath, action), services);
