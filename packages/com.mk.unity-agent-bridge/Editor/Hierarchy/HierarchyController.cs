...
namespace Mk.UnityAgentBridge.Editor.Hierarchy
{
    internal static class HierarchyController
    {
        ...
        {
            CapabilityRegistry.Declare("hierarchy");
            RouteTable.Register("GET", "hierarchy/roots", ctx => Roots());
            RouteTable.Register("GET", "hierarchy/tree", ctx => Tree(ctx));
            RouteTable.Register("GET", "hierarchy/find", ctx => Find(ctx));
            RouteTable.Register("GET", "hierarchy/ancestors", ctx => Ancestors(ctx));
            RouteTable.Register("GET", "hierarchy/inspect", ctx => Inspect(ctx));
        }
