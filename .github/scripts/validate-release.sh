...
read -r PACKAGE_VERSION PYPROJECT_VERSION INIT_VERSION <<< "$(python3 - "$REPO_ROOT" <<'PY'
import json
import re
import sys
...
print(f"{package_version} {pyproject_match.group(1)} {init_match.group(1)}")
PY
)"
...
if [[ "$TAG_VERSION" != "$PACKAGE_VERSION" ]] \
  ...
  echo "版本不一致：tag=${TAG_VERSION}, package.json=${PACKAGE_VERSION}, pyproject.toml=${PYPROJECT_VERSION}, __init__.py=${INIT_VERSION}"
  exit 1
fi
