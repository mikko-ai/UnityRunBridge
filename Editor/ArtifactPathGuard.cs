...
namespace Mk.UnityAgentBridge.Editor
{
    ...
    /// </summary>
    internal static class ArtifactPathGuard
    {
        ...
        {
            string directory;
            if (SessionController.HasActiveSession && !string.IsNullOrEmpty(SessionController.CurrentSessionPath))
            {
                directory = Path.Combine(SessionController.CurrentSessionPath, ArtifactsSubDirName);
            }
            else
            {
        ...
        {
            int seq = SessionController.HasActiveSession
        ...
        {
            string key = $"Mk.UnityAgentBridge.ArtifactSeq.{SessionController.CurrentSessionId}.{prefix}";
