...
{
    /// <summary>
    /// Session 状态服务（Phase 1 从 SessionController 抽出到 Core）：持有当前 session 的
    /// id/path/日志写入器与序号，负责启动/结束/恢复与 Play Mode 运行边界记录。
    /// EditorStateProvider、ArtifactPathGuard 只依赖本服务，不再依赖路由层的 SessionController；
    /// SessionController 退化为路由登记 + 薄转发。status JSON 字段保持不变。
    /// </summary>
