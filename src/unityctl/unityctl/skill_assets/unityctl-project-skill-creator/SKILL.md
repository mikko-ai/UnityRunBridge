...
5. **工具依赖契约**：每个 flow 的生成物必须声明对官方 unityctl skill 的依赖（先读 `.agents/skills/unityctl/SKILL.md`，再按实际能力读对应 `references/*.md`）；禁止把官方命令手册复制进生成物，参数、前置条件与错误处理一律以官方 skill / reference 为准。
6. **内置自愈**：生成物末尾必须包含「自愈指引」一节（模板见各 flow）。
